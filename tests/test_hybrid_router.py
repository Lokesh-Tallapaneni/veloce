...
def test_plain_tree_paths_are_not_regex():
...
def test_bare_param_matches_one_segment():
...
def test_tree_routes_still_match():
    r = Router()
...
def test_no_regex_routes_leaves_match_unchanged():
    r = Router()
...
def test_tree_wins_over_regex_on_overlap():
    r = Router()
...
def test_param_in_segment_matches():
    r = Router()
...
def test_raw_regex_converter_matches_and_rejects():
    r = Router()
...
def test_greedy_path_with_suffix_matches():
    r = Router()
...
def test_multi_brace_segment_matches():
    r = Router()
...
def test_registration_order_first_match_wins():
    r = Router()
...
def test_method_mismatch_reports_allowed_methods():
    r = Router()
...
def test_url_for_resolves_named_regex_route():
    r = Router()
...
def test_url_for_multi_brace_segment():
    r = Router()
...
def test_url_for_param_in_segment():
    r = Router()
...
def test_include_merges_regex_routes():
    main = Router()
...
def test_regex_route_appears_in_schema_with_openapi_path():
    app = Veloce()
...
def test_multi_brace_route_openapi_path():
    app = Veloce()
...
def test_collect_all_routes_includes_regex():
    r = Router()
...
def test_partial_segment_unknown_converter_raises():
...
def test_partial_segment_known_converter_is_regex():
    assert is_regex_path("/v{version:int}/api") is True


def test_multi_brace_unknown_converter_raises():
    with pytest.raises(ValueError, match="unknown path converter"):
        is_regex_path("/files/{name:bogus}.{ext}")


def test_unknown_converter_route_registration_raises():
    r = Router()
...
def test_known_converter_route_registers_and_matches():
    r = Router()
...
def test_unknown_converter_in_later_whole_segment_raises():
...
def test_unknown_converter_in_later_segment_route_registration_raises():
    r = Router()
...
def test_unknown_converter_in_partial_later_segment_raises():
...
def test_int_param_in_segment_is_coerced():
    r = Router()
...
def test_float_param_is_coerced():
    r = Router()
...
def test_uuid_param_is_coerced():
    r = Router()
...
def test_any_param_stays_string():
    r = Router()
...
def test_raw_regex_param_stays_string():
    r = Router()
...
def test_regex_int_honors_digit_cap_like_radix():
...
def test_regex_int_overlong_is_404_not_405():
...
def test_regex_int_at_cap_still_matches():
...
def test_is_regex_path_with_inner_braces():
    assert is_regex_path("/x/{id:[0-9]{2}}") is True
    assert is_regex_path("/x/{id:\\d{2}}") is True


def test_bracket_quantifier_matches_two_digits():
...
def test_escaped_digit_quantifier_matches_two_digits():
    pat = build_route_regex("/x/{id:\\d{2}}")
    assert pat.match("/x/42").groupdict() == {"id": "42"}
    assert pat.match("/x/4") is None


def test_route_with_inner_braces_dispatches():
    r = Router()
...
def test_inner_brace_route_openapi_path():
    app = Veloce()
...
def test_inner_brace_route_url_for():
    r = Router()
...
def test_tolerant_accepts_extra_trailing_slash():
    r = Router()
...
def test_tolerant_accepts_missing_trailing_slash():
    r = Router()
...
def test_strict_regex_route_rejects_extra_slash():
    r = Router()
...
def test_tolerant_allowed_methods_with_extra_slash():
    r = Router()
...
def test_tree_handler_wins_over_regex_in_schema():
    app = Veloce()
...
def test_collect_all_routes_drops_shadowed_regex():
    r = Router()
