...
- **`verify_password` refuses tampered scrypt cost parameters** (#85).
  ...
  rejects any stored hash whose `N`, `r`, or `p` is below the cost
  floor `hash_password` itself uses.
...
- **`FileResponse(path)` refuses to block the event loop** (#79). The
  sync constructor previously did an `open(path).read()` on the
  running loop, stalling every concurrent request for the duration of
  the read. The constructor now raises a clear `RuntimeError` when a
  loop is running and directs callers to `await
  FileResponse.from_path(path)`. Sync construction still works in
  scripts / CLI tools that run outside a loop.
...
- **Router + middleware decorator stop using `assert` as a runtime
  guard** (#87). Both `router.py:487` (radix-tree converter pin) and
  `app.py:950` (middleware decorator type check) were stripped under
